...
static std::string BuildFsShaderInline(const CCFeatures& cc_features, FilteringMode filter_mode, bool srgb_mode) {
    ...
    /* Battleship-vita shader-compile-failure investigation (2026-08-20):
     * hookTexture2D (and filter3point, in the FILTER_THREE_POINT branch)
     * were emitted unconditionally here, but every *call* site below is
     * gated by cc_features.usedTextures[i] - so a shader combination with
     * no active texture units (confirmed real case: a noise-dither
     * post-process shader generated by StartDrawToFramebuffer) got a dead
     * hookTexture2D definition with an unused sampler2D parameter. That
     * reliably produced "Shader Compiler: E] fatal internal error on line
     * -1." from SceShaccCg on real Vita hardware, followed by a data abort
     * inside SceGxm - confirmed via device coredumps and live capture,
     * ruled out mod()/fmod() semantics and coroutine stack size as causes
     * first. Desktop/mobile GL drivers apparently tolerate the dead
     * function fine (this shipped and worked on other platforms), but
     * SceShaccCg does not. Only emit these when a texture is actually
     * bound, matching the call sites' own condition. */
    if (cc_features.usedTextures[0] || cc_features.usedTextures[1]) {
