...
Interpreter::Interpreter() {
...
#ifdef __vita__
    /* A release VPK can carry the GXP files generated by one cold hardware
     * run. Reading those immutable files directly from app0 removes the
     * multi-minute SceShaccCg pass even on a clean install. Development VPKs
     * without the bundle keep vitaGL's default ux0 path so they can generate
     * and persist the cache that will later be packaged. */
    const char* shaderCacheSource = "ux0-runtime";
    SceIoStat vertexCacheStat = {};
    SceIoStat fragmentCacheStat = {};
    if (sceIoGetstat("app0:/shader_cache/v0/v", &vertexCacheStat) >= 0 &&
        sceIoGetstat("app0:/shader_cache/v0/f", &fragmentCacheStat) >= 0) {
        vglSetShaderCachePath("app0:/shader_cache");
        shaderCacheSource = "app0-precompiled";
    }
    ...
    port_log("SSB64: vitaGL config param=4MiB circular=8MiB display_buffers=2 "
             "ram_threshold=32MiB msaa=1x shader_cache=%s\n", shaderCacheSource);
