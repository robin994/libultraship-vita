...
void Interpreter::Run(Gfx* commands, const robin_hood::unordered_map<Mtx*, MtxF>& mtx_replacements) {
    ...
    sVitaFast3DStats.run_us += vitaRunUs;
    if (vitaWalkUs > sVitaFast3DStats.walk_us_max) {
        sVitaFast3DStats.walk_us_max = vitaWalkUs;
    }
    if (vitaRunUs > sVitaFast3DStats.run_us_max) {
        sVitaFast3DStats.run_us_max = vitaRunUs;
    }
    if (sVitaFast3DStats.frames >= 300) {
        port_log("SSB64: FAST3D frames=%u commands=%llu cmd_per_frame=%u flushes=%llu "
                 "walk_us_avg=%u walk_us_max=%u run_us_avg=%u run_us_max=%u\n",
                 sVitaFast3DStats.frames,
                 (unsigned long long)sVitaFast3DStats.commands,
                 (uint32_t)(sVitaFast3DStats.commands / sVitaFast3DStats.frames),
                 (unsigned long long)sVitaFast3DStats.flushes,
                 (uint32_t)(sVitaFast3DStats.walk_us / sVitaFast3DStats.frames),
                 sVitaFast3DStats.walk_us_max,
                 (uint32_t)(sVitaFast3DStats.run_us / sVitaFast3DStats.frames),
                 sVitaFast3DStats.run_us_max);
        sVitaFast3DStats = {};
    }
#endif
}
